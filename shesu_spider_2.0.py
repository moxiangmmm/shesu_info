...
class ZhixingSpider:
    ...
    def _search_company(self, company, captcha_url):
        ...
        # print(post_data)
        resp = requests.post('http://zhixing.court.gov.cn/search/newsearch', data=post_data, headers=headers,
        ...
        if len(tr_list) > 1:
            ...
            for tr in tr_list:
                ...
                if len(id) > 0:
                    ...
                    detail_url = "http://zhixing.court.gov.cn/search/newdetail?id={}&j_captcha={}&captchaId=fda97538121240b38b0c73eeac144dbe&_={}".format(
                        id, captcha, time_id)
                    # 发送请求
                    try:
    ...
    def run_text(self):
        ...
        with ThreadPoolExecutor(2) as executor:
            for company in company_list:
                future = executor.submit(self._run,company )
                futures.add(future)
        try:
